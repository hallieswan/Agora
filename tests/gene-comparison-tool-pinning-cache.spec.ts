...
import {
  gene2WithMultipleProteinsTMT,
  geneWithMultipleProteinsTMT,
} from './helpers/data';
import {
  changeGctCategory,
  changeGctSubcategory,
  expectGctPageLoaded,
} from './helpers/gct';
import {
...
test.describe('GCT: Caching pinned genes', () => {
  test('Pinned genes are maintained when switching between RNA subcategories', async ({
    page,
  }) => {
    ...
    async ({ page }) => {
      ...
      await test.step('pin genes in RNA category', async () => {
        ...
        await test.step('clear all items', async () => {
          ...
          await expect(page.getByText(/Pinned Genes/i)).not.toBeVisible();
        });
    ...
    async ({ page }) => {
      ...
      await test.step('pin proteins in Protein category', async () => {
        ...
        await test.step('confirm pinned items', async () => {
          ...
          await confirmPinnedProteins(
            page,
            proteinCategoryGene,
            proteinCategoryProteins
          );
        });
      });
